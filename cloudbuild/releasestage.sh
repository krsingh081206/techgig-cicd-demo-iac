...
javabackendimage=`jq -r '.staging | .springbootsvc' /workspace/cloudbuild/$release/release.json`
...
sed -i "s/image: springbootbackend/image: us-east4-docker.pkg.dev\/$projectid\/techgig-cicd\/$javabackendimage/g" /workspace/cloudbuild/cicd-demo-backend-java/k8s/staging/api-deployment.yaml
...
cat /workspace/cloudbuild/cicd-demo-backend-java/k8s/staging/api-deployment.yaml
